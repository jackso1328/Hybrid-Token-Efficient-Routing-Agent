...
# Allowed models for the hackathon (Fallback to free Zephyr if others are offline)
_allowed_models_str = os.environ.get("ALLOWED_MODELS", "")
ALLOWED_MODELS = [m.strip() for m in _allowed_models_str.split(",")] if _allowed_models_str else ["huggingfaceh4/zephyr-7b-beta:free"]
