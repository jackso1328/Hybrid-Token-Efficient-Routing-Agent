...
def verify_math_answer(prompt: str, gemma_answer: str, local_engine=None) -> Tuple[bool, Optional[str]]:
    ...
    if local_engine and local_engine.is_loaded:
        expression_prompt = f"""You are a calculator. The user provides a math problem and its answer.
Output ONLY the final mathematical calculation as a single Python expression. Do not output anything else.
Example: 80 * (1 - 0.25)

Problem: {prompt}
Claimed answer: {gemma_answer}"""
    ...
    # Try SymPy for symbolic math if simple eval didn't work and sympy is available
    if local_engine and local_engine.is_loaded and sympy:
        sympy_prompt = f"""Convert this math problem to a SymPy script that prints the final numeric answer.
